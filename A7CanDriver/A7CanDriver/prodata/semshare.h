...
public:
    ...
    Sem_Qt_Data(uint sz, key_t semkey = 19860610, QString sharekey="dykshare")
    {
        creat_sem_data(sz,semkey, sharekey);
    }
    ~Sem_Qt_Data(){
    ...
    int creat_sem_data(uint sz, key_t semkey = 19860610, QString sharekey="dykshare",int al = 8);
    int write_send_data(T val);
    int read_send_data(T & val);
    int wait_thread_sem(void);
};
